...
# 品番設定JSON読込処理 --------------------
def load_product_settings(file_name):
    with open(file_name, encoding="utf-8") as f:
        return json.load(f)


# PLC管理番号から品番を取得 --------------------
def get_product_name(settings, plc_no):
    for product_name, data in settings.items():
        if data["plc_no"] == plc_no:
            return product_name
...
def input_count_val():
    ...
    # PLC管理番号から品番を取得
    plc_no = int(get_from_plc(ipadd, "DM900"))
    ...
    if product_name is None:
        messagebox.showerror("エラー", f"PLC管理番号 {plc_no} が登録されていません。")
        return

    lbl_product["text"] = product_name

    # 加工機番の選択肢を設定
    machine_no_list = product_settings[product_name]["process_machine_no"]
    cmb_mcno["values"] = machine_no_list
    var_mcno.set("")

    # 検査数格納
    val = get_from_plc(ipadd, "DM602.D")
    txt_total.delete(0, tk.END)
    txt_total.insert(tk.END, val)
    # OK数格納
    val = get_from_plc(ipadd, "DM604.D")
    txt_ok.delete(0, tk.END)
    txt_ok.insert(tk.END, val)
    # NG1(形状NG)数格納
    val = get_from_plc(ipadd, "DM612.D")
    txt_ng1.delete(0, tk.END)
    txt_ng1.insert(tk.END, val)
    # NG2(側面NG)数格納
    val = get_from_plc(ipadd, "DM614.D")
    txt_ng2.delete(0, tk.END)
    txt_ng2.insert(tk.END, val)
    # NG3(端面NG)数格納
    val = get_from_plc(ipadd, "DM616.D")
    txt_ng3.delete(0, tk.END)
    txt_ng3.insert(tk.END, val)
    # NG4(全長NG)数格納
    val = get_from_plc(ipadd, "DM618.D")
    txt_ng4.delete(0, tk.END)
    txt_ng4.insert(tk.END, val)
    # NG5(予備NG)数格納
    val = get_from_plc(ipadd, "DM620.D")
    val = 0  # ＜注＞ 使用する時に削除すること
    txt_ng5.delete(0, tk.END)
    txt_ng5.insert(tk.END, val)
    # NG6(予備NG)数格納
    val = get_from_plc(ipadd, "DM620.D")
    val = 0  # ＜注＞ 使用する時に削除すること
    txt_ng6.delete(0, tk.END)
    txt_ng6.insert(tk.END, val)
...
def get_present_data():
    ...
    lst.append(lbl_product["text"]) # 品番
...
def get_csv_path(mc_no, ins_date):
    ...
    fname = "MC" + str(mc_no) + "_" + ins_date[:7].replace("/", "") + ".CSV"
...
def input_modifydata():
    ...
    lbl_product["text"] = val[1]  # 品番
    var_mcno.set(val[2])  # 加工機番
    txt_from.insert(tk.END, val[3])  # 加工日開始
    txt_to.insert(tk.END, val[4])  # 加工日終了
    var_insno.set(val[5])  # 検査機番
    txt_date.insert(tk.END, val[6])  # 検査日
    txt_total.insert(tk.END, val[7])  # 検査数
    txt_ok.insert(tk.END, val[8])  # OK数
    txt_ng1.insert(tk.END, val[9])  # NG1数
    txt_ng2.insert(tk.END, val[10])  # NG2数
    txt_ng3.insert(tk.END, val[11])  # NG3数
    txt_ng4.insert(tk.END, val[12])  # NG4数
    txt_ng5.insert(tk.END, val[13])  # NG5数
    txt_ng6.insert(tk.END, val[14])  # NG6数
    txt_note.insert(tk.END, val[15])  # 備考
    # 修整中フラグセット＆ラベル表示
    global modify_flg
    modify_flg = True
    lbl_modify["text"] = "--- 修整中 ---"
...
def create_buwindow():
    ...
    cmb_drv = ttk.Combobox(
        buwin,
        width=7,
        font=("", 12),
        state="readonly",
        textvariable=var_drv,
        values=["D:", "E:", "F:", "G:"],
    )
    cmb_drv.place(x=230, y=20)
    # バックアップ対象選択：LabelFrame+Radiobutton x2
    bu_frame = ttk.Labelframe(buwin, text="バックアップ対象選択", width=320, height=80)
    bu_frame.place(x=20, y=60)
    var_butarget = tk.IntVar()
    tk.Radiobutton(
        bu_frame, value=1, variable=var_butarget, text="全データ", font=("", 12)
    ).place(x=20, y=10)
    tk.Radiobutton(
        bu_frame,
        value=2,
        variable=var_butarget,
        text="最新ファイルのみ",
        font=("", 12),
    ).place(x=140, y=10)
    # 実行＆閉じるボタン：BUtton x2
...
product_settings = load_product_settings("product_settings.json")
...
frame1 = ttk.Labelframe(
    root, text="登録用データ", width=1500, height=180, style="TLabelframe"
)
...
txt_id = ttk.Label(
    frame1, text="", font=("", 12), relief="solid", width=11, anchor=tk.CENTER
)
...
cmb_mcno2 = ttk.Combobox(
    frame1,
    width=7,
    font=("", 12),
    state="readonly",
    textvariable=var_insno,
    values=cmb_list,
)
...
frame2 = ttk.Labelframe(
    root, text="データ一覧：", width=1500, height=565, style="TLabelframe"
)
frame2.pack(padx=10, pady=10)

# 列の識別名設定(識別名は後から使わないので単なる数字とする)
clm_id = [n for n in range(16)]
# Treeview生成
...
# 列幅&書式設定(データ構造に依存)
tree.column(0, anchor="center", width=100)  # DataID
tree.column(1, anchor="center", width=140)  # 品番
tree.column(2, anchor="center", width=80)   # 加工機番
tree.column(3, anchor="center", width=110)  # 加工日開始
tree.column(4, anchor="center", width=110)  # 加工日終了
tree.column(5, anchor="center", width=80)   # 検査機番
tree.column(6, anchor="center", width=110)  # 検査日
tree.column(7, anchor="center", width=70)   # 検査数
tree.column(8, anchor="center", width=70)   # OK数
tree.column(9, anchor="center", width=70)   # NG1
tree.column(10, anchor="center", width=70)  # NG2
tree.column(11, anchor="center", width=70)  # NG3
tree.column(12, anchor="center", width=70)  # NG4
tree.column(13, anchor="center", width=70)  # NG5
tree.column(14, anchor="center", width=70)  # NG6
tree.column(15, anchor="w", width=150)      # 備考
